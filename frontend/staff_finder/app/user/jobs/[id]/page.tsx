        ...
        <div className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-2xl p-5 space-y-4">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-gray-900 mt-1">Rs. {salary.toLocaleString()}</p>
            </div>
            <hr className="border-gray-100" />
            <button
              onClick={() => router.push(`/user/jobs/${id}/apply`)}
              className="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold py-3 rounded-xl text-sm transition-colors"
            >
              Apply Now
            </button>
            <button className="w-full border border-gray-200 text-gray-700 font-semibold py-3 rounded-xl text-sm hover:bg-gray-50 transition-colors">
              🔖 Save Job
            </button>
          </div>
        </div>
...
function Chip({ label }: { label: string }) {
  return (
    <span className="text-xs font-semibold px-3 py-1 rounded-full capitalize bg-orange-100 text-orange-700">
      {label}
    </span>
  );
}
