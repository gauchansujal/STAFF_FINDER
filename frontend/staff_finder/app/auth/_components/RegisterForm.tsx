...
// ── Schema (inline so this file is self-contained) ──────────────────────────
...
export default function RegisterForm({ onSubmit, onNavigateToLogin }: RegisterFormProps) {
  ...
  // Handle file pick → convert to base64 data URL for preview
  // (In production you'd upload to storage and get back a URL)
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      // Store as data URL in imageUrl field; swap for upload URL in production
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-6">
      <div className="w-full max-w-md bg-white border border-black rounded-xl shadow-sm">
        ...
        {/* ── Avatar at top, overlapping the card edge ── */}
        <div className="flex flex-col items-center pt-8 pb-2 px-8">
          <button
            type="button"
            onClick={() => fileInputRef.current?.click()}
            className="relative w-20 h-20 rounded-full border-2 border-black bg-gray-100
              overflow-hidden cursor-pointer group focus:outline-none
        ...
        <div className="px-8 pb-8">
          ...
          {/* Server Error */}
          {serverError && (
            <div className="border border-red-200 rounded-lg px-4 py-3 text-sm
              text-red-700 bg-red-50 mb-5">
              ⚠ {serverError}
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              {derivedUsername.length > 1 && (
                <p className="text-xs text-gray-400 -mt-1">
                  Username will be{" "}
                  <span className="font-semibold text-gray-600">@{derivedUsername}</span>
                </p>
              )}
              ...
              <div>
                ...
                <div className="relative flex items-center">
                  ...
                  <button
                    ...
                    className="absolute right-3 text-xs font-semibold text-gray-500
              ...
              <button
                ...
                className={`w-full py-3 bg-black text-white text-sm font-semibold
                  rounded-lg border border-black transition-colors mt-2
                  ${isSubmitting
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            ...
            <button
              ...
              className="text-black font-semibold underline cursor-pointer
                bg-transparent border-none text-sm hover:text-gray-700 transition-colors"
            >
              Sign in
            </button>
          </p>
